...
void handleRestore() {
  ...
  if (err) {
    Serial.print("[restore] JSON invalide : ");
  ...
  if (written != body.length()) {
    Serial.println("[restore] écriture incomplète !");
    server.send(500, "text/plain", "écriture partielle");
    return;
  }
  ...
  if (check) {
    ...
    check.close();
    Serial.print("[restore] taille du fichier après écriture : ");
    Serial.println(size);
    if (size != body.length()) {
      Serial.println("[restore] taille incohérente !");
      server.send(500, "text/plain", "taille erronée");
      return;
    }
  } else {
