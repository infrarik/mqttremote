...
// ------------------------------------------------------------------ état réseau
// 4 états possibles pour la machine d'état réseau
enum NetState { NET_OK, NET_MQTT_KO, NET_WIFI_KO, NET_RECONNECTING };
NetState net_state = NET_RECONNECTING;

unsigned long last_wifi_attempt  = 0;
unsigned long last_mqtt_attempt  = 0;
unsigned long wifi_backoff_ms    = 5000;   // démarre à 5s, monte jusqu'à 30s
unsigned int  reconnect_count    = 0;      // compteur affiché sur la page
...
void load_config() {
  ...
  if (deserializeJson(json, f) == DeserializationError::Ok) {
    ...
    strcpy(btn1_topic,          json["btn1_topic"]          | "esp/btn1/topic");
    strcpy(btn1_payload,        json["btn1_payload"]        | "message_btn1");
    strcpy(btn1_name,           json["btn1_name"]           | "UP");
    strcpy(btn1_color,          json["btn1_color"]          | "green");
    strcpy(btn1_long_topic,     json["btn1_long_topic"]     | "");
    strcpy(btn1_long_payload,   json["btn1_long_payload"]   | "");
    strcpy(btn2_topic,          json["btn2_topic"]          | "esp/btn2/topic");
    strcpy(btn2_payload,        json["btn2_payload"]        | "message_btn2");
    strcpy(btn2_name,           json["btn2_name"]           | "MY");
    strcpy(btn2_color,          json["btn2_color"]          | "white");
    strcpy(btn2_long_topic,     json["btn2_long_topic"]     | "");
    strcpy(btn2_long_payload,   json["btn2_long_payload"]   | "");
    strcpy(btn3_topic,          json["btn3_topic"]          | "esp/btn3/topic");
    strcpy(btn3_payload,        json["btn3_payload"]        | "message_btn3");
    strcpy(btn3_name,           json["btn3_name"]           | "DOWN");
    strcpy(btn3_color,          json["btn3_color"]          | "red");
    strcpy(btn3_long_topic,     json["btn3_long_topic"]     | "");
    strcpy(btn3_long_payload,   json["btn3_long_payload"]   | "");
...
// Appelé dans le loop — clignotement non bloquant selon net_state
void update_led() {
  unsigned long now = millis();
  unsigned long interval = 0;

  switch (net_state) {
    case NET_OK:           interval = 0;    break;  // LED éteinte fixe
    case NET_MQTT_KO:      interval = 800;  break;  // clignotement lent
    case NET_WIFI_KO:      interval = 150;  break;  // clignotement rapide
    case NET_RECONNECTING: interval = 300;  break;  // clignotement moyen
  }

  if (interval == 0) {
    digitalWrite(PIN_LED, HIGH);  // éteinte (active LOW)
    led_state = false;
    return;
  }
...
// Génère la card d'un bouton dans le setup
// n = 1/2/3, pin_lbl = "D1"/"D2"/"D5"
String btn_card(int n,
                const char* pin_lbl,
                const char* name,   const char* color,
                const char* topic,  const char* payload,
                const char* ltopic, const char* lpayload) {
  String p   = String(n);
  String dot = "dot" + p;
  String s;
...
void handle_setup() {
  ...
  html += btn_card(1, "D1", btn1_name, btn1_color, btn1_topic, btn1_payload, btn1_long_topic, btn1_long_payload);
  html += btn_card(2, "D2", btn2_name, btn2_color, btn2_topic, btn2_payload, btn2_long_topic, btn2_long_payload);
  html += btn_card(3, "D5", btn3_name, btn3_color, btn3_topic, btn3_payload, btn3_long_topic, btn3_long_payload);
  html += F("</div>");
  ...
  html += F("<script>"
    ...
    "function doSave(){"
      ...
      "var names=['mqtt_server','mqtt_port','mqtt_user','mqtt_pass','long_ms',"
        "'b1_name','b1_color','b1_t','b1_p','b1_lt','b1_lp',"
        "'b2_name','b2_color','b2_t','b2_p','b2_lt','b2_lp',"
        "'b3_name','b3_color','b3_t','b3_p','b3_lt','b3_lp'];"
...
void handle_save() {
  ...
  strcpy(mqtt_pass,           server.arg("mqtt_pass").c_str());
  strcpy(btn1_topic,          server.arg("b1_t").c_str());
  strcpy(btn1_payload,        server.arg("b1_p").c_str());
  strcpy(btn1_name,           server.arg("b1_name").c_str());
  strcpy(btn1_color,          server.arg("b1_color").c_str());
  strcpy(btn1_long_topic,     server.arg("b1_lt").c_str());
  strcpy(btn1_long_payload,   server.arg("b1_lp").c_str());
  strcpy(btn2_topic,          server.arg("b2_t").c_str());
  strcpy(btn2_payload,        server.arg("b2_p").c_str());
  strcpy(btn2_name,           server.arg("b2_name").c_str());
  strcpy(btn2_color,          server.arg("b2_color").c_str());
  strcpy(btn2_long_topic,     server.arg("b2_lt").c_str());
  strcpy(btn2_long_payload,   server.arg("b2_lp").c_str());
  strcpy(btn3_topic,          server.arg("b3_t").c_str());
  strcpy(btn3_payload,        server.arg("b3_p").c_str());
  strcpy(btn3_name,           server.arg("b3_name").c_str());
  strcpy(btn3_color,          server.arg("b3_color").c_str());
  strcpy(btn3_long_topic,     server.arg("b3_lt").c_str());
  strcpy(btn3_long_payload,   server.arg("b3_lp").c_str());
...
void manage_network() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    // Tentative de reconnexion WiFi avec backoff (5s → 10s → 30s max)
...
void check_buttons() {
  ...
  if ((now - last_debounce_time1) > debounce_delay) {
    if (r1 == LOW) {
      if (!pressed1) {
        // Front descendant : action courte immédiate + début chrono appui long
        pressed1    = true;
        long_fired1 = false;
        press_start1 = now;
        Serial.println("[action] D1 appuyé (court)");
        publish_if_connected(btn1_topic, btn1_payload, "btn1-court");
      }
      // Détection appui long (une seule fois)
      if (!long_fired1 && (now - press_start1) >= long_press_ms) {
        long_fired1 = true;
        Serial.println("[action] D1 appui long");
